...
from pandapipes.utils.internals import PipeflowNotConverged, set_user_pf_options
...
def test_set_user_pf_options(create_test_net, use_numba):
    ...
    set_user_pf_options(net, reset=True, hello='world')
    ...
    set_user_pf_options(net, reset=True, hello='world')
    set_user_pf_options(net, reset=True)
    ...
    # priority
    set_user_pf_options(net, reset=True, tol_p=1e-6, tol_m=1e-6)
...
def test_iter(create_test_net, use_numba):
    ...
    set_user_pf_options(net, iter=2)
    ...
    set_user_pf_options(net, max_iter_hyd=max_iter_hyd)
    pandapipes.pipeflow(net, mode='hydraulics')

    with pytest.raises(PipeflowNotConverged):
        pandapipes.pipeflow(net, mode='sequential')

    set_user_pf_options(net, max_iter_therm=max_iter_therm)

    pandapipes.pipeflow(net, mode='sequential')

    set_user_pf_options(net, max_iter_hyd=max_iter_hyd, max_iter_therm=max_iter_therm)
