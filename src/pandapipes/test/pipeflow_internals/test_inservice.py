...
def create_test_net():
    ...
    pandapipes.create_valve(net, j1, j4, 'j', 0.1)
    pandapipes.create_valve(net, j4, j5, 'j', 0.1, opened=False)
...
def complex_heat_connectivity_grid():
    ...
    pandapipes.create_valve(net, j5, j6, 'j',0.1, index=10)
    pandapipes.create_valve(net, j4, j5, 'j',0.1, opened=False, index=12)
...
def create_mixed_indexing_grid():
    ...
    pandapipes.create_pipes_from_parameters(
        ...
        index=[0, 3, 10, 7, 6, 8])
    pandapipes.create_valves(net, [3, 10, 6], [14, 12, 15], 'j', 0.2, index=[3, 5, 2])
