...
def _calculate_A_B(a_p, a_T, a_molar_fraction,  a_p_crit, a_T_crit, a_acent_fact):
    ...
    shape = np.shape(a_p)
    T_red = np.zeros((len(a_p_crit)*len(a_p)))
    p_red = np.zeros((len(a_p_crit) * len(a_p)))
    fluid_node_combination = 0

    for node in range(len(a_p)):
        T_red[fluid_node_combination] = a_T[node] / a_T_crit[0]
        T_red[fluid_node_combination+1] = a_T[node] / a_T_crit[1]
        p_red[fluid_node_combination] = a_p[node] / a_p_crit[0]
        p_red[fluid_node_combination+1] = a_p[node] / a_p_crit[1]
        fluid_node_combination += 2
...
def calculate_mixture_compressibility_draft(mf, p, T):
    ...
    #T_crit = np.array([T_crit_meth, T_crit_h2])
    T_crit = np.full([shape[0],2], np.array([T_crit_meth, T_crit_h2]))
    #p_crit = np.array([p_crit_meth, p_crit_h2])
    p_crit = np.full([shape[0], 2], np.array([p_crit_meth, p_crit_h2]))
    #acent_fact = np.array([acent_fact_meth, acent_fact_h2])
    acent_fact = np.full([shape[0], 2],np.array([acent_fact_meth, acent_fact_h2]))

    res_comp = newton(_func_of_Z, np.array([0.9, 0.9]), fprime=_der_func_of_Z, args=(p, T, mf))
    res_comp_norm = newton(_func_of_Z, np.array([0.9, 0.9]), fprime=_der_func_of_Z, args=(p_n, T_n))
