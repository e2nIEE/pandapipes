...
def _calculate_A_B(a_molar_fraction, a_p, a_T, a_p_crit, a_T_crit, a_acent_fact):
    ...
    sqrt_alpha = (1 + m * (1 - T_red(a_T) ** 0.5))
...
def _func_of_Z(Z):
    A_mixture, B_mixture = _calculate_A_B
    return Z ** 3 - Z ** 2 + Z *(A_mixture - B_mixture - B_mixture ** 2) - A_mixture * B_mixture

def _der_func_of_Z(Z, p_n):
    A_mixture, B_mixture = _calculate_A_B
    return  3 * Z **2 - 2 * Z + (A_mixture - B_mixture - B_mixture **2)

def calculate_mixture_compressibility_draft(p_n):
    res_comp = newton(_func_of_Z, 0.9 ,fprime=_der_func_of_Z, args=())
    res_comp_norm = newton(_der_func_of_Z, 0.9, fprime=_der_func_of_Z, args=(p_n))
...
compr_mixture = calculate_mixture_compressibility_draft(molar_fraction, p, T, p_crit, T_crit, acent_fact)
