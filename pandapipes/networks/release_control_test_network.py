...
path = os.path.join(pp_dir, "networks", "simple_test_networks")


def release_control_test_network(use_json=False, use_time_series=False):
    if use_json is False:
        ...
        # pump
        pp.create_pump_from_parameters(net, from_junction=8, to_junction=3, pump_name="Pump",
                                       pressure_list=[6.1, 5.8, 4],
                                       flowrate_list=[0, 19, 83], regression_degree=2,
    ...
    elif use_json is True and use_time_series is False:
        net = pp.from_json(os.path.join(path, "release_control_test_network.json"))
        return net

    else:
        net = pp.from_json(os.path.join(path, "release_control_test_network_with_controller.json"))
        return net
