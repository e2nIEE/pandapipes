...
def test_tee_pipe():
    ...
    nodes=4
    pp.create_pipe_from_parameters(net, j1, j2, 1, 75e-3, k_mm=.1, sections=sections, alpha_w_per_m2k=5, text_k=293.15)
    pp.create_pipe_from_parameters(net, j2, j3, 1, 75e-3, k_mm=.1, sections=sections, alpha_w_per_m2k=5, text_k=293.15)
    pp.create_pipe_from_parameters(net, j2, j4, 1, 75e-3, k_mm=.1, sections=sections, alpha_w_per_m2k=5, text_k=293.15)
    ...
    pipe1[1:-1,:] = np.transpose(copy.deepcopy(res_T[:,nodes:nodes+(sections-1)]))
    pipe2[1:-1,:] = np.transpose(copy.deepcopy(res_T[:,nodes+(sections-1):nodes+(2*(sections-1))]))
    pipe3[1:-1,:] = np.transpose(copy.deepcopy(res_T[:,nodes+(2*(sections-1)):nodes+(3*(sections-1))]))

    datap1 = pd.read_csv("C:\\Users\\dcronbach\\pandapipes\\pandapipes\\non_git\\Temperature.csv", sep=';',
    ...
    line1, = ax.plot(np.arange(0,sections+1,1)*1000/sections, pipe1[:,10], color = "black", marker="+")
    line11, = ax.plot(np.arange(0,sections+1,1)*1000/sections, pipe1[:,30], color = "black",linestyle="dotted")
    line12, = ax.plot(np.arange(0,sections+1,1)*1000/sections, pipe1[:,90], color = "black",linestyle="dashdot")
    #d1 = ax.plot(np.arange(0,sections+1,1)*1000/sections, datap1["T"], linestyle="dashed", color = "black")
    line2, = ax1.plot(np.arange(0,sections+1,1)*1000/sections, pipe2[:,10],  color = "black", marker="+")
    line21, = ax1.plot(np.arange(0,sections+1,1)*1000/sections, pipe2[:,30],  color = "black", linestyle="dotted")
    line22, = ax1.plot(np.arange(0,sections+1,1)*1000/sections, pipe2[:,90],  color = "black", linestyle="dashdot")
    #d2 = ax1.plot(np.arange(0,sections+1,1)*1000/sections, datap2["T"], color = "black", linestyle="dashed")
    line3, = ax2.plot(np.arange(0,sections+1,1)*1000/sections, pipe3[:,10], color = "black", marker="+")
