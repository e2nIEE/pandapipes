...
from pandapipes.idx_branch import FROM_NODE, TO_NODE, TINIT_IN, ELEMENT_IDX, RHO, ETA, CP, ACTIVE
...
class BranchWOInternalsComponent(BranchComponent):
    ...
    def create_pit_branch_entries(cls, net, branch_pit):
        ...
        branch_wo_internals_pit[:, TINIT_IN] = (node_pit[from_nodes, TINIT_NODE]
                                             + node_pit[to_nodes, TINIT_NODE]) / 2
        fluid = get_fluid(net)
        branch_wo_internals_pit[:, RHO] = fluid.get_density(branch_wo_internals_pit[:, TINIT_IN])
        branch_wo_internals_pit[:, ETA] = fluid.get_viscosity(branch_wo_internals_pit[:, TINIT_IN])
        branch_wo_internals_pit[:, CP] = fluid.get_heat_capacity(branch_wo_internals_pit[:, TINIT_IN])
