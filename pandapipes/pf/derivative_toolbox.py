...
def derivatives_hydraulic_incomp_np(net, branch_pit, der_lambda, p_init_i_abs, p_init_i1_abs,
                                    height_difference):
    v_init_abs = np.abs(branch_pit[:, net['_idx_branch']['VINIT']])
    v_init2 = v_init_abs * branch_pit[:, net['_idx_branch']['VINIT']]
    lambda_term = np.divide(branch_pit[:, net['_idx_branch']['LENGTH']] *
                            branch_pit[:, net['_idx_branch']['LAMBDA']],
                            branch_pit[:, net['_idx_branch']['D']]) + \
                            branch_pit[:, net['_idx_branch']['LOSS_COEFFICIENT']]
    const_p_term = np.divide(branch_pit[:, net['_idx_branch']['RHO']], P_CONVERSION * 2)
    df_dv = const_p_term * (2 * v_init_abs * lambda_term + der_lambda
                            * np.divide(branch_pit[:, net['_idx_branch']['LENGTH']],
                                        branch_pit[:, net['_idx_branch']['D']]) * v_init2)
    load_vec = p_init_i_abs - p_init_i1_abs + branch_pit[:, net['_idx_branch']['PL']] \
               + const_p_term * (GRAVITATION_CONSTANT * 2 * height_difference
                                 - v_init2 * lambda_term)
    mass_flow_dv = branch_pit[:, net['_idx_branch']['RHO']] * \
                   branch_pit[:, net['_idx_branch']['AREA']]
    df_dv_nodes = mass_flow_dv
    load_vec_nodes = mass_flow_dv * branch_pit[:, net['_idx_branch']['VINIT']]
...
def derivatives_hydraulic_comp_np(net, branch_pit, lambda_, der_lambda, p_init_i_abs, p_init_i1_abs,
                                  height_difference, comp_fact, der_comp, der_comp1):
    # Formulas for gas pressure loss according to laminar version
    v_init_abs = np.abs(branch_pit[:, net['_idx_branch']['VINIT']])
    v_init2 = branch_pit[:, net['_idx_branch']['VINIT']] * v_init_abs
    ...
    const_lambda = np.divide(NORMAL_PRESSURE *
                             ...
                             branch_pit[:, net['_idx_branch']['TINIT']],
    ...
    const_height = np.divide(
        ...
        2 * NORMAL_PRESSURE * branch_pit[:, net['_idx_branch']['TINIT']] * P_CONVERSION)
    friction_term = np.divide(lambda_ * branch_pit[:, net['_idx_branch']['LENGTH']],
                              branch_pit[:, net['_idx_branch']['D']]) + \
                              branch_pit[:, net['_idx_branch']['LOSS_COEFFICIENT']]

    load_vec = p_diff + branch_pit[:, net['_idx_branch']['PL']] + const_height * p_sum \
    ...
    df_dv = np.divide(2 * const_lambda * comp_fact, p_sum) * v_init_abs * friction_term \
            + np.divide(const_lambda * comp_fact * der_lambda * branch_pit[:, net['_idx_branch']['LENGTH']] * v_init2,
                        p_sum * branch_pit[:, net['_idx_branch']['D']])
    mass_flow_dv = branch_pit[:, net['_idx_branch']['RHO']] * branch_pit[:, net['_idx_branch']['AREA']]
    df_dv_nodes = mass_flow_dv
    load_vec_nodes = mass_flow_dv * branch_pit[:, net['_idx_branch']['VINIT']]
...
def calc_derived_values_np(net, node_pit, from_nodes, to_nodes):
    tinit_branch = (node_pit[from_nodes, net['_idx_node']['TINIT']] +
                    node_pit[to_nodes, net['_idx_node']['TINIT']]) / 2
    height_difference = node_pit[from_nodes, net['_idx_node']['HEIGHT']] - \
                        node_pit[to_nodes, net['_idx_node']['HEIGHT']]
    p_init_i_abs = node_pit[from_nodes, net['_idx_node']['PINIT']] + \
                   node_pit[from_nodes, net['_idx_node']['PAMB']]
    p_init_i1_abs = node_pit[to_nodes, net['_idx_node']['PINIT']] + \
                    node_pit[to_nodes, net['_idx_node']['PAMB']]
